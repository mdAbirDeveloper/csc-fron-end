...
const index = ({ data }) => {
  return (
    <div>
      <Head>
        <title>حسِّن أنظمتك الهيدروليكية باستخدام صمامات عالية الجودة</title>
      </Head>
      ...
      <div className="grid lg:grid-cols-5 md:grid-cols-5 grid-cols-2 gap-4  mx-auto">
        {data.map((hydrulicvalves) => (
          <>
            <div>
              <div className="card bg-base-100 shadow-xl">
                <figure>
                  <img src={hydrulicvalves?.images[0]} alt="Shoes" />
                </figure>
                <div className="card-body">
                  <h2 className="text-center text-xl font-bold font-serif">{hydrulicvalves?.name}</h2>
                  <p></p>
                  <div className="card-actions justify-center">
                    <Link href={`/components/hydraulicValves/${hydrulicvalves._id}`}>
