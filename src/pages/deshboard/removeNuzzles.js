...
const removeNuzzles = ({data}) => {
  const router = useRouter();
  const handleDelete = async (id) => {
    console.log(id)
    try {
      await fetch(`https://csc-server-again.vercel.app/fogNuzzles/${id}`, {
        method: "DELETE",
      })
  ...
  return (
    ...
    <h1 className="text-3xl my-8 text-center font-bold uppercase text-green-500"> remove your Nuzzles</h1>
      {data.map((products) => (
        <>
...
removeNuzzles.getLayout = function getLayout(page) {
  return (
