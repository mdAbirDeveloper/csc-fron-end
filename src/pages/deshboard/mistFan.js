...
const mistFan = () => {
  ...
  const onSubmit = async (data) => {
    ...
    for (let i = 0; i < data.images.length; i++) {
      ...
      setSubmited("mistFan added successfully")
      setLoading(false);
    ...
    try {
      ...
      const response = await fetch("https://csc-server-again.vercel.app/mistFan", {
        method: "POST",
        headers: {
    ...
    <div>
      ...
      <form className="card-body" onSubmit={handleSubmit(onSubmit)}>
        <div className="form-control">
          <input
            type="text"
            placeholder="write your mistFan old price"
        ...
        <div className="form-control">
          <input
            type="text"
            placeholder="write your mistFan price"
            className="input input-bordered"
            {...register("price")}
          />
        </div>
        <div className="form-control">
          <input
            type="text"
            placeholder="write your mistFan name"
            className="input input-bordered"
            required
            {...register("name")}
          />
        ...
        <label className="text-red-400">user , for give a line brack in your descriptions</label>
          <textarea
            type="text"
            placeholder="write your mistFan description"
            className="input input-bordered"
            required
        ...
        <div>
          <label>Choose images of your mistFan</label>
...
export default mistFan;


mistFan.getLayout = function getLayout(page) {
  return (
