...
const removeConnectors = ({data}) => {
  const router = useRouter();
  const handleDelete = async (id) => {
    console.log(id)
    try {
      await fetch(`https://csc-server-again.vercel.app/connectors/${id}`, {
        method: "DELETE",
      })
  ...
  return (
    ...
    <h1 className="text-3xl my-8 text-center font-bold uppercase text-green-500"> remove your connectors</h1>
      {data.map((products) => (
        <>
...
export default removeConnectors;
...
removeConnectors.getLayout = function getLayout(page) {
  return (
