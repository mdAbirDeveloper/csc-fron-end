...
const removeValves = ({data}) => {
  const router = useRouter();
  const handleDelete = async (id) => {
    console.log(id)
    try {
      await fetch(`https://csc-server-again.vercel.app/hydrulicValves/${id}`, {
        method: "DELETE",
      })
  ...
  return (
    ...
    <h1 className="text-3xl my-8 text-center font-bold uppercase text-green-500"> remove your Valves</h1>
      {data.map((products) => (
        <>
...
removeValves.getLayout = function getLayout(page) {
  return (
